...
{
    if (len > sizeof(etx_value) - 1) {
        pr_err("ERROR: Data too large to store in etx_value\n");
        return -EINVAL;
    }
    ...
    etx_value[len] = '\0';  // Ensure null-termination
...
static int __init etx_driver_init(void)
{
  ...
  if((alloc_chrdev_region(&dev, 0, 1, "etx_Dev_select")) <0)
  {
  ...
  if(IS_ERR(dev_class = class_create("etx_class_select")))
  {
  ...
  /*Creating device*/
  if(IS_ERR(device_create(dev_class,NULL,dev,NULL,"etx_device_select")))
  {
  ...
  kobj_ref = kobject_create_and_add("etx_sysfs_select",kernel_kobj);
  ...
  pr_info("Select Linux Device Driver Insert...Done!!!\n");
  return 0;
r_sysfs:
...
static void __exit etx_driver_exit(void)
{
  ...
  cdev_del(&etx_cdev);
  unregister_chrdev_region(dev, 1);
  pr_info("Select Linux Device Driver Remove...Done!!!\n");
}
...
MODULE_DESCRIPTION("Simple linux driver Select Linux");
MODULE_VERSION("1.41");
