...
int thread_function2(void *pv)
{
    ...
    while(!kthread_should_stop()) {
        ...
        pr_info("In Seqlock Kernel  Thread Function 2 : Read value %lu\n", read_value);
        msleep(1000);
    }
    return 0;
}
...
static int __init etx_driver_init(void)
{
        ...
        if((alloc_chrdev_region(&dev, 0, 1, "etx_Dev_seq")) <0){
        ...
        if(IS_ERR(dev_class = class_create("etx_class_seq"))){
            pr_info("Cannot create the struct class\n");
            goto r_class;
        }
 
        /*Creating device*/
        if(IS_ERR(device_create(dev_class,NULL,dev,NULL,"etx_device_seq"))){
        ...
        /* Creating Thread 1 */
        etx_thread1 = kthread_run(thread_function1,NULL,"eTx Thread1_seq");
        if(etx_thread1) {
         ...
         /* Creating Thread 2 */
        etx_thread2 = kthread_run(thread_function2,NULL,"eTx Thread2_seq");
        if(etx_thread2) {
        ...
        pr_info("Seqlock Device Driver Insert...Done!!!\n");
        return 0;
...
static void __exit etx_driver_exit(void)
{
        ...
        cdev_del(&etx_cdev);
        unregister_chrdev_region(dev, 1);
        pr_info("Seqlock Device Driver Remove...Done!!\n");
}
 
/* Module Init and Kernel */
