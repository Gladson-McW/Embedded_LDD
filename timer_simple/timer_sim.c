/* Implementation of Basic Kernel Based Timer */
...
static int __init etx_driver_init(void)
{
    ...
    if((alloc_chrdev_region(&dev, 0, 1, "etx_Dev_tima")) <0){
    ...
    if(IS_ERR(dev_class = class_create("etx_class_tima"))){
        pr_err("Cannot create the struct class\n");
        goto r_class;
    }
 
    /*Creating device*/
    if(IS_ERR(device_create(dev_class,NULL,dev,NULL,"etx_device_tima"))){
    ...
    pr_info("Kernel Timer Device Driver Insert...Done!!!\n");
    return 0;
r_device:
...
static void __exit etx_driver_exit(void)
{
    ...
    cdev_del(&etx_cdev);
    unregister_chrdev_region(dev, 1);
    pr_info("Kernel Timer Device Driver Remove...Done!!!\n");
}
...
MODULE_DESCRIPTION("Kernel Timer Device Driver ");
MODULE_VERSION("1.21");