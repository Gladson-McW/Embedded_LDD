/* Implementation of Process File System */
...
#include <linux/cdev.h>            // Character Driver Registration
#include <linux/device.h>
#include <linux/slab.h>                 //kmalloc()
#include <linux/uaccess.h>              //copy_to/from_user()
#include <linux/ioctl.h>       // for input output control
#include <linux/proc_fs.h>     // for process file system
...
/* Kernel Version of Raspberry Pi 5 is 6.6.74+rpi-rpi-2712 
 * SO LINUX_KERNEL_VERSION IS 606
*/
#define LINUX_KERNEL_VERSION  606
/* IOCTL VALUE WITH 32 BIT INTEGER ENTRY */
...
static int __init etx_driver_init(void)
{
        ...
        if(IS_ERR(dev_class = class_create("etx_class_procfs"))){
        ...
        pr_info("Device Driver (Procfs) Insert...Done!!!\n");
        return 0;
...
static void __exit etx_driver_exit(void)
{
        ...
        cdev_del(&etx_cdev);
        unregister_chrdev_region(dev, 1);
        pr_info("Device Driver (Procfs) Remove...Done!!!\n");
}
...
MODULE_DESCRIPTION("Process Function (Procfs) with IOCTL Character Driver");
MODULE_VERSION("1.6");