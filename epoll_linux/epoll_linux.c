...
static int __init etx_driver_init(void)
{
  ...
  if((alloc_chrdev_region(&dev, 0, 1, "etx_Dev_epoll")) <0)
  {
  ...
  if(IS_ERR(dev_class = class_create("etx_class_epoll")))
  {
  ...
  /*Creating device*/
  if(IS_ERR(device_create(dev_class,NULL,dev,NULL,"etx_device_epoll")))
  {
  ...
  kobj_ref = kobject_create_and_add("etx_sysfs_epoll",kernel_kobj);
  ...
  pr_info("E-Poll Linux Device Driver Insert...Done!!!\n");
  return 0;
r_sysfs:
...
static void __exit etx_driver_exit(void)
{
  ...
  cdev_del(&etx_cdev);
  unregister_chrdev_region(dev, 1);
  pr_info("E-Poll Linux Device Driver Remove...Done!!!\n");
}
...
MODULE_DESCRIPTION("E-Poll Linux Device Driver");
MODULE_VERSION("1.41");